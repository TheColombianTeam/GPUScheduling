...
def save_csv(data_x, data_y, filename):
    csv_file = ["x,y\n"]
    for idx in range(len(data_x)):
        csv_file.append(f"{data_x[idx]},{data_y[idx]}\n")
    with open(filename, "w") as file:
        file.writelines(csv_file)
...
logger.warning(
    "Studing dynamic behaviout of Schedulers for gpu structure --> number of clusters : "
    + str(args.gpu.cluster)
    + "  number of SM : "
    + str(args.gpu.sm)
)
...
for fault in range(1000):
    ...
    for CTA in CTAs_2lrr:
        if CTA["Cluster"] == faulty_Cluster and CTA["SM"] == faulty_SM:
            for _x in range(MS):
                for _y in range(NS):
                    if (CTA["CTA"]["x"] + _x) < dimentions and (
                        CTA["CTA"]["y"] + _y
                    ) < dimentions:
                        x_2lrr.append(
                            CTA["CTA"]["x"] + _x
                        )  # fault propagation is confined in CTA dimentions
...
path_csv = os.path.join(os.getcwd(), "DynamicBehaviour", "TwoLevelRoundRobin" + ".csv")
save_csv(x_2lrr, y_2lrr, path_csv)

path_csv = os.path.join(os.getcwd(), "DynamicBehaviour", "GlobalRoundRobin" + ".csv")
save_csv(x_grr, y_grr, path_csv)

path_csv = os.path.join(os.getcwd(), "DynamicBehaviour", "Greedy" + ".csv")
save_csv(x_greedy, y_greedy, path_csv)

path_csv = os.path.join(os.getcwd(), "DynamicBehaviour", "DistributedBlock" + ".csv")
save_csv(x_db, y_db, path_csv)

path_csv = os.path.join(os.getcwd(), "DynamicBehaviour", "DistributedCTA" + ".csv")
save_csv(x_dcta, y_dcta, path_csv)
