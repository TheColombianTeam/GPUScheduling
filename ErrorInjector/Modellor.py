...
class Modellor(ErrorInjector,MaskInjector):
    def __init__(self,Scheduler,imported = True, update = False):#imported falg is specifing if we are importing in a bigger env with respect to Schedulers
        ...
        self.__scheduler = self._SchedulerAllocationMethod(self._Scheduler)
        if not(self._update):
                ...
                for coordinate in Masks[int(CTA % len(Masks))] :  #Masks is a list of dictionary {str(x,y) : Mask}
                    ...
                    try :
                        Faulty_np[CTA['CTA']['x'] + _x][CTA['CTA']['y'] + _y ] = self.MaskInjection.ApplyMask( self, 
                                        Float16(Faulty_np[CTA['CTA']['x'] + _x][CTA['CTA']['y'] + _y ]), self._Masks[int(CTA % len(Masks))][coordinate])
    ...
    def AssignFaultToModel(self,Masks,AvgAbsError,MaxAbsError,MinAbsError, FaultID):
        for Model in range(len(self._Models)):
            if (0.1 < self._Models[Model]['AvgAbsError']/AvgAbsError < 10 and
                ...
                Associate = True
                for coordinate in self._Models[Model][0] : #Masks is a dictionary {(x', y') : Mask}-->Generated by Mask injector--> Tested by ErrorInjector -->Assigned to a model or generate a model
                ...
                if Associate :
                    self._Models[Model]['FaultsAssocited'].append(FaultID)
                    return Associate
    ...
    def AddModel(self,FaultID,Masks, AvgAbsError,MaxAbsError,MinAbsError):
        ...
        self._Models[-1]['FaultsAssocited'].append(FaultID)
