...
def injector(
    ...
    d_faulty = faulty_Kernel_execution(
        a, b, c, d_golden, CTA, int(fault[1]), int(fault[2]), int(fault[3])
    )
    (
        faulty_entrances,
        falty_values,
        golden_faulty_values,
        faulty_hex,
        golden_faulty_hex,
    ) = corrupted_etrances_extrapolation(d_faulty, d_golden)
    ...
    for entrance in range(len(faulty_entrances)):
        ...
        shared_faults_Queue.put(
            (
                ...
                x,
                y,
                golden_faulty_values[entrance],
                golden_faulty_hex[entrance],
                falty_values[entrance],
                faulty_hex[entrance],
            )
        )
