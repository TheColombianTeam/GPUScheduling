...
def golden(A_x_dim, A_B_common_dim, B_y_dim, min_value=0, max_value=1):
    ...
    a = create_matrix(
        size=[A_x_dim, A_B_common_dim], scale=(max_value - min_value), offset=-min_value
    )
    b = create_matrix(
        size=[A_B_common_dim, B_y_dim], scale=(max_value - min_value), offset=-min_value
    )
    c = create_matrix(
        size=[A_x_dim, B_y_dim], scale=(max_value - min_value), offset=-min_value
    )
    d_ = tiling(a, b, c)
    d = np.matmul(a, b) + c
    ...
    CTA_allocation_and_scheduler_vaidation(
        TwoLevelRoundRobin_scheduler, a, b, c, d_
    )  # since SFPY uses float16 is reccomended that from now on the golden values are not those  generated from np.matmul since an instrisic error generated by using float32 should not be taken into consideration during fault injection
