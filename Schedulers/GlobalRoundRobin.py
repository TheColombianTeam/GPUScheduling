    ...
    def scheduler_algorithm(self, a, b, c):
        ...
        for CTA_id in range(len(self.__scheduler_info)):
            if( CTA_id < self.CTAs_to_schedule_statically_in_clusters):
                ...
                sm_counter += 1
                if(sm_counter == self._n_SM_per_cluster):
                    sm_counter = 0
                    cluster_id +=1
                if(cluster_id == self._n_clusters):
                    cluster_id = 0
            else : #dynamic scheduler
                if(CTA_id == (self.CTAs_to_schedule_statically_in_clusters + dynamic_scheduled_row *self._n_clusters*self._n_SM_per_cluster)):
                    sm_counter = 0
                    cluster_id = 0
                    stocastic_ordering = []
        ...
        for c in range(len(to_schedule_CTAs_per_cluster)):
            ...
            for CTA in range(n_CTA_for_this_cluster):
                if(CTA < self.CTAs_to_schedule_statically_in_SM):
                    ...
                    if(CTA == (self.CTAs_to_schedule_statically_in_SM + dynamic_scheduled_row*self._n_SM_per_cluster)):
                        SM_id = 0
                        stocastic_ordering = []
