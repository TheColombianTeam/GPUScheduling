    ...
    def scheduler_algorithm(self, a, b, c):
        ...
        for CTA_id in range(len(self.__scheduler_info)):
            if(CTA_id < self.CTAs_to_schedule_statically_in_clusters):
                ...
                if(CTA_id == self.CTAs_to_schedule_statically_in_clusters+dynamic_cluster_filled*self.CTAs_to_schedule_statically_in_clusters):
                    cluster_id = 0
                    stocastic_ordering = []
        ...
        for c in range(len(to_schedule_CTAs_per_cluster)):
            ...
            for CTA in range(n_CTA_for_this_cluster):
                if(CTA < self.CTAs_to_schedule_statically_in_SM):
                    ...
                    if(CTA == self.CTAs_to_schedule_statically_in_SM+dynamic_block*self.CTAs_to_schedule_statically_in_SM):
                        SM_id = 0
                        stocastic_order = []
                        stocastic_order = self.random_delay_generator_simulator(self._n_SM_per_cluster)
         
                    self.__scheduler_info[to_schedule_CTAs_per_cluster[c][CTA]]["SM"] = stocastic_order[SM_id]
                    SM_id += 1
